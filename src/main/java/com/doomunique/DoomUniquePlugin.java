...
public class DoomUniquePlugin extends Plugin implements RenderCallback
{
	...
	@Inject
	private DoomUniqueOverlay overlay;
	...
	private final Set<TileObject> testOverlayObjects = Collections.newSetFromMap(new IdentityHashMap<>());
	private final Set<String> discoverySeenObjects = new HashSet<>();
	private final Set<String> statusMessages = new HashSet<>();

	private Set<Integer> uniqueHoleIds = Collections.emptySet();
	private Set<Integer> normalHoleIds = Collections.emptySet();
	...
	protected void shutDown()
	{
		...
		clientThread.invokeLater(() ->
		{
			resetAllModels();
			clearTestSwaps();
			testOverlayObjects.clear();
			discoverySeenObjects.clear();
			statusMessages.clear();
			uniqueHoleIds = Collections.emptySet();
			normalHoleIds = Collections.emptySet();
	...
	private void refreshObjectIds()
	{
		...
		normalHoleIds = ObjectIdParser.parse(config.normalHoleObjectIds());
	...
	private void scanSceneForHoles()
	{
		...
		try
		{
			...
			int[] matchedTestObjects = new int[1];
			forEachSceneObject(object ->
			{
				...
				if (isConfiguredTestObject(object))
				{
					matchedTestObjects[0]++;
				}
				handleSceneObjectSafely(object);
			});

			if (isTestModeEnabled() && matchedTestObjects[0] == 0)
			{
				if (matchedTestObjectIds[0] > 0)
				{
					messageOnce("test-type-no-match:" + testObjectIds + ":" + config.testObjectType(),
						"Temporary test IDs matched loaded objects, but none were " + config.testObjectType() + ".");
				}
				else
				{
					messageOnce("test-no-match:" + testObjectIds,
						"No loaded scene objects matched Temporary test object IDs " + testObjectIds + ".");
				}
			}
		}
	...
	private boolean hasConfiguredSceneTargets()
	{
		return config.recolorUniqueHole() && !uniqueHoleIds.isEmpty()
			|| config.recolorTestObjects() && !testObjectIds.isEmpty()
			|| config.testOnNormalHole() && !normalHoleIds.isEmpty();
	}
	...
	Set<TileObject> getTestOverlayObjects()
	{
		return testOverlayObjects;
	}

	private void handleSceneObject(TileObject object)
	{
		trackTestOverlayIfMatched(object);
	...
	{
		return config.recolorTestObjects() && !testObjectIds.isEmpty();
	}

	private void trackTestOverlayIfMatched(TileObject object)
	{
		if (object == null || !config.testOnNormalHole() || !normalHoleIds.contains(object.getId()))
		{
			return;
		}

		testOverlayObjects.add(object);
	}
	...
	private Model getUniqueHoleSwapModel()
	{
		...
		try
		{
			...
			if (uniqueHoleSwapModel != null && config.recolorSwapTestObjects())
			{
				...
				if (recoloredFaces == 0 && config.recolorMode() == RecolorMode.GOLD_FACES)
				{
				...
				{
					messageOnce("swap-recolored:" + config.swapTargetObjectId() + ":" + config.recolorMode(),
	...
	private Color colorFor(TileObject object)
	{
		...
		if (config.recolorTestObjects() && isConfiguredTestObject(object))
		{
			messageOnce("recolor:" + objectId + ":" + objectType(object), "Recolor active for "
				+ objectType(object) + " test object ID " + objectId + ".");
			return config.uniqueHoleColor();
		}
	...
	private int recolorModel(Model model, Color color)
	{
		...
		if (config.recolorMode() == RecolorMode.ALL_FACES)
		{
			int recoloredFaces = maxFaceCount(model);
			fill(model.getFaceColors1(), rs2hsb);
			fill(model.getFaceColors2(), rs2hsb);
			fill(model.getFaceColors3(), rs2hsb);
			fill(model.getUnlitFaceColors(), (short) rs2hsb);
			if (config.removeTextures())
			{
				fill(model.getFaceTextures(), (short) -1);
			}
		...
		if (config.removeTextures())
		{
			removeTextures(model.getFaceTextures(), changedFaces);
		}
		return recoloredFaces;
	}
	...
	private void markMutedInteriorFaces(Model model, boolean[] changedFaces)
	{
		...
		float innerRadius = maxRadius * 0.52f;
		float innerRadiusSquared = innerRadius * innerRadius;
		int faceCount = Math.min(model.getFaceCount(), changedFaces.length);
		faceCount = Math.min(faceCount, Math.min(faceIndices1.length, Math.min(faceIndices2.length, faceIndices3.length)));
		for (int face = 0; face < faceCount; face++)
		{
			if (changedFaces[face] || !isMutedInteriorFace(model, face)
	...
	private void removeTextures(short[] textures, boolean[] changedFaces)
	{
		if (textures == null)
		{
			return;
	...
	{
		testOverlayObjects.remove(object);
	...
	private void discoverNearbyObjects()
	{
		...
		String filter = config.discoveryNameFilter().trim().toLowerCase();
		int radius = config.discoveryRadius();

		forEachSceneObject(object ->
		{
			WorldPoint objectPoint = object.getWorldLocation();
			if (!isWithinRadius(playerPoint, objectPoint, radius))
			{
				return;
			}

			String name = getObjectName(object.getId());
			...
			String animationInfo = objectAnimationInfo(object);
			String key = type + ":" + object.getId() + ":" + animationInfo + ":" + objectPoint.getPlane()
				+ ":" + objectPoint.getX() + ":" + objectPoint.getY();
			if (!discoverySeenObjects.add(key))
			{
				return;
			}

			String message = String.format("Doom Unique discovery: type=%s id=%d%s name=\"%s\" world=(%d,%d,%d)",
				type, object.getId(), animationInfo, name, objectPoint.getX(), objectPoint.getY(), objectPoint.getPlane());
			log.info(message);
			if (config.discoveryChatMessages())
			{
	...
	private void discoverNearbyObjectsSafely()
	{
		try
		{
			discoverNearbyObjects();
		}
		catch (RuntimeException | AssertionError ex)
		{
			log.warn("Unable to discover Doom Unique nearby objects", ex);
			messageOnce("discovery-object-error", "Object discovery failed once; skipping this scan. See client.log.");
		}
	}
	...
	private void fill(int[] colors, int value)
	{
		if (colors != null)
		{
	...
	private void fill(short[] colors, short value)
	{
		if (colors != null)
		{
			Arrays.fill(colors, value);
